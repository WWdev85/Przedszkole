...
export const AdvertisementForm = ({advertisement ,index, changes}) =>{
    ...
    const{ dispatchGroupAdvertisements} = useContext(StoreContext);
    ...
    const [group] = useState(advertisement.group);
    ...
    const handleDeleteAdvertisement = () =>{
        dispatchGroupAdvertisements({type: 'DELETE', index: index});
        changes(Math.random());
    }

    const handleSubmit = (e) => {
        e.preventDefault();
        const newAdvertisement = {id,title, content, createdAt: date, file, group};

        if(!(index >= 0)){
            dispatchGroupAdvertisements({type: 'ADD', data: newAdvertisement});
            setDate("");
            setContent("");
            setFile(null);
            changes(Math.random() );
        }else{  
            dispatchGroupAdvertisements({type: 'CHANGE', data: newAdvertisement, index: index });
            changes(Math.random());
        }
    ...
    return (
        ...
        <form className={style('form')} onSubmit={handleSubmit} ref={form} >
            ...
            <div className={style('input-wrapper')}>
                <input className={style('input-file')} onChange={changeFile} type="file"  accept="image/png, image/jpeg, image/jpg, application/pdf, application/doc, application/docx, video/mp4/*" />
